...
def getIndexes(current_p, path, k, used_index, avoid, indexes, direction, dif, begins, ends, let_cycle):
    def getIndexesDown(current_p, path, k, used_index, avoid, indexes):
        prev_len = len(indexes)
        for index, item  in enumerate(current_p):
            if item == path[k] and index != used_index and index%2 == 0 and not index in avoid and not current_p[index+1] in path[-1] and let_cycle == 0:
    ...
    def getIndexesUp(current_p, path, k, used_index, avoid, indexes):
        prev_len = len(indexes)
        for index, item  in enumerate(current_p):
            if item == path[k] and index != used_index and index%2 == 1 and not index in avoid and not current_p[index+1] in path[-1] and let_cycle == 0:
                indexes.append(index)
        print(indexes)
        dif = len(indexes) - prev_len
        return(dif)
    ...
    if discriminator:
        ...
        for begin in begins:
            ...
            while begin != None:
                ...
                k = 1
                let_cycle = 0
                indexes = [index for index, item in enumerate(current_p) if path[k] == item and index%2 == 0 and index != begin+1 and not index in avoid and not current_p[index+1] in path[-1] and let_cycle == 0]
                dif = len(indexes)
                if indexes:
                    for index in indexes:
                        k += 1
                        if dif == 1:
                            ...
                            if current_p[used_index] in path and let_cycle == 0:
                                let_cycle = 1
                            path.append(current_p[used_index])
                            dif, direction = getIndexes(current_p, path, k, used_index, avoid, indexes, direction, dif, begins, ends, let_cycle)
        ...
        for end in ends:
                ...
                while end != None:
                    ...
                    k = 1
                    let_cycle = 0
                    indexes = [index for index, item in enumerate(current_p) if path[k] == item and index%2 == 1 and index != end-1 and not index in avoid and not current_p[index-1] in path[-1] and let_cycle==0]
                    dif = len(indexes)
                    if indexes:
                        for index in indexes:
                            k += 1
